...
# SYNTAX: /regex language/ =~ "string"
require 'pry'

def has_sin?(string)
 sin_match = /\b\d\d\d-\d\d\d-\d\d\d\b/.match(string) 
  if sin_match
    return true 
  else 
    return false 
...
#puts has_sin?("please don't share this: 234-604-142")

#puts "has_sin? returns false if it doesn't have a SIN"
#puts has_sin?("please confirm your identity: XXX-XXX-142")
#puts has_sin?("please don't share this: 234-6043-142") 
#puts has_sin?("please don't share this: 2342-604-142") 
#puts has_sin?("please don't share this: 234-604-1421") 

# Return the Social Insurance Number from a string.
        #GSUB SYNTAX: gsub(pattern, replacement) OR gsub(pattern){|match|}
def grab_sin(string)
  sin_match = /\b\d\d\d-\d\d\d-\d\d\d\b/.match(string)
  if sin_match
    return 
